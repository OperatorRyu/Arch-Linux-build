...
chrome.webRequest.onBeforeSendHeaders.addListener(
  request => {
    ...
    const GPCEnabled = deserialize(options.gpcEnabled) || false
    if (GPCEnabled) {
      //console.log("adding GPC Header")
...
function onBeforeReq(details) {
  // if(isRestrictedUrl(details.url)){
  //   console.log("restricted url... returning")
  //   return
  // }
        ...
        var ID = setInterval(function() {
          if (IS_INITIALIZED()) {
            ...
            TABS.query({}, function(tabs) {
              ...
              for (var i = 0; i < TAB_COUNT; i++) {
                var tab = tabs[i];
                console.log("Added a parent domain", tab.url)
                DOMAINS[tab.id] = GET(tab.url);
              }
            });
          }
...
chrome.webRequest.onBeforeRequest.addListener(onBeforeReq , {urls: ['http://*/*', 'https://*/*']}, ['blocking']);
...
// based on initial implementation provided by 
function injectDOMSignal (tabId) {
  const browserName = window.navigator.userAgent.indexOf("Firefox") >= 0 ? "moz" : "chrome"// returns 'chrome' for Chromium forks or 'moz' for Firefox
  const GPCEnabled = deserialize(options.gpcEnabled) 
  ...
  chrome.tabs.executeScript(tabId, {
    file: `scripts/GPC/GPC-${browserName}.js`,
    allFrames: true,
    matchAboutBlank: true,
    runAt: 'document_start'
  })
